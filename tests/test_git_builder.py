...
class TestSingleRepoWorktree:
    """Tests for --single-repo / worktree functionality."""

    def test_creates_orphan_branch_in_source_repo(self, tmp_path):
        """Should create an orphan branch in the source repo."""
        ...
        (source_repo_path / "README.md").write_text("# Source Project")
        source_repo.index.add(["README.md"])
        source_repo.index.commit("Initial commit in source")

        # Set up the worktree output directory
        worktree_path = tmp_path / "worktree_output"
        ...
        # Worktree should exist at output_dir
        ...
        try:
            source_repo.merge_base(main_commit, orphan_branch.commit)
            has_common_ancestor = True
        except Exception:
            has_common_ancestor = False

        # An orphan branch has no merge base with main - the merge_base will be empty
        merge_bases = source_repo.merge_base(main_commit, orphan_branch.commit)
        assert len(merge_bases) == 0, "Orphan branch should have no common ancestor with main"

    def test_worktree_points_to_source_repo(self, tmp_path):
        """Worktree should be linked to source repo, not a separate repo."""
        source_repo_path = tmp_path / "source_repo"
        source_repo_path.mkdir()
        source_repo = Repo.init(source_repo_path)
        with source_repo.config_writer() as config:
        ...
        (source_repo_path / "README.md").write_text("# Source")
        source_repo.index.add(["README.md"])
        ...
        # The worktree's .git should be a file pointing to the main repo
        git_path = worktree_path / ".git"
        assert git_path.exists()
        # Worktrees have a .git file (not directory) that points to main repo
        assert git_path.is_file(), ".git should be a file for worktrees"

        git_content = git_path.read_text()
        assert "gitdir:" in git_content
        assert "source_repo" in git_content or str(source_repo_path) in git_content

    def test_incremental_build_with_worktree(self, tmp_path):
        """Incremental builds should work with worktree setup."""
        source_repo_path = tmp_path / "source_repo"
        source_repo_path.mkdir()
        source_repo = Repo.init(source_repo_path)
        with source_repo.config_writer() as config:
        ...
        initial_commits = len(list(repo1.iter_commits()))

        # Second build with additional operation
        ops2 = [
            FileOperation(
                file_path="/project/file1.py",
                operation_type=OperationType.WRITE,
        ...
        # Should only add 1 new commit
        assert final_commits == initial_commits + 1
        assert (worktree_path / "file2.py").exists()

    def test_reuses_existing_worktree(self, tmp_path):
        """Should reuse existing worktree if already set up."""
        source_repo_path = tmp_path / "source_repo"
        source_repo_path.mkdir()
        source_repo = Repo.init(source_repo_path)
        with source_repo.config_writer() as config:
        ...
        # Second build should reuse it without error
        builder2 = GitRepoBuilder(
            output_dir=worktree_path,
            source_repo=source_repo_path,
            branch="agentgit/history",
    ...
    def test_branch_exists_adds_worktree(self, tmp_path):
        """Should add worktree to existing branch if branch exists but no worktree."""
        source_repo_path = tmp_path / "source_repo"
        source_repo_path.mkdir()
        source_repo = Repo.init(source_repo_path)
        with source_repo.config_writer() as config:
        ...
        # Switch back to main
        source_repo.git.checkout("main" if "main" in source_repo.heads else "master")
        ...
        # Should have both the existing file and new file
        assert (worktree_path / "existing.py").exists()
        assert (worktree_path / "new.py").exists()

    def test_without_orphan_creates_regular_branch(self, tmp_path):
        """Without orphan=True, should create regular branch from main."""
        source_repo_path = tmp_path / "source_repo"
        source_repo_path.mkdir()
        source_repo = Repo.init(source_repo_path)
        with source_repo.config_writer() as config:
        ...
        builder = GitRepoBuilder(
            output_dir=worktree_path,
            source_repo=source_repo_path,
            branch="feature/agentgit",
            orphan=False,
        )
        repo, _, _ = builder.build(ops)

        # Branch should have main as ancestor
        main_commit = source_repo.heads.main.commit if "main" in source_repo.heads else source_repo.heads.master.commit
        feature_branch = source_repo.heads["feature/agentgit"]

        merge_bases = source_repo.merge_base(main_commit, feature_branch.commit)
        assert len(merge_bases) > 0, "Regular branch should share ancestor with main"
